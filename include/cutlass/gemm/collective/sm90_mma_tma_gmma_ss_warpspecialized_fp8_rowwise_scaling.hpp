...
struct CollectiveMma<
    MainloopSm90TmaGmmaWarpSpecializedRowWiseFP8<Stages, ClusterShape, KernelSchedule>,
    TileShape_,
    ElementA_,
    StrideA_,
    ElementB_,
    ...
    TransformB_>
{
    ...
    // Type Aliases
    //
    using DispatchPolicy = MainloopSm90TmaGmmaWarpSpecializedRowWiseFP8<Stages, ClusterShape, KernelSchedule>;
    ...
    using ElementScale = ElementAccumulator;
    ...
    using ScalerCopyTypeA = cute::uint_byte_t<cute::min(static_cast<int>(sizeof(ElementScale)) * size<0>(TileShape{}), 16)>;
    using SmemScalingCopyAtomA = Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS<ScalerCopyTypeA>, ElementScale>;
    using SmemScalingCopyAtomB = Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS<ElementScale>, ElementScale>;

    static constexpr int BLK_M = size<0>(TileShape{});
    static constexpr int BLK_N = size<1>(TileShape{});
    // Block scaling smem layout
    using SmemLayoutScaleA = Layout<Shape<Int<BLK_M>>>; // (BLK_M)
    using SmemLayoutScaleB = Layout<Shape<Int<BLK_N>>>; // (BLK_N)
    ...
    static_assert(cute::is_same_v<GmemTiledCopyB, SM90_TMA_LOAD> || cute::is_same_v<GmemTiledCopyB, SM90_TMA_LOAD_MULTICAST>,
        "GmemTiledCopy - invalid SM90 TMA copy atom specified.");
    static_assert(cute::is_same_v<ElementAccumulator, ElementScale>,
              "ElementAccumulator and ElementScale should be same datatype");
    ...
    struct SharedStorage
    {
      ...
      cute::array_aligned<ElementScale, cute::cosize_v<SmemLayoutScaleB>> smem_scale_B; // (BLK_N)
    } tensors;
  ...
  load_init(ProblemShape_MNKL const& problem_shape_MNKL, Params const& mainloop_params) const {
    ...
    Tensor mScaleA_mkl = make_tensor(make_gmem_ptr(mainloop_params.ptr_scale_A), scaleA_layout);  // (M,1):(1,M)
    Tensor mScaleB_nkl = make_tensor(make_gmem_ptr(mainloop_params.ptr_scale_B), scaleB_layout);  // (N,1):(1,N)
  ...
  load(
    ...
    if (lane_predicate) {
      ...
      Tensor sScaleA = make_tensor(cute::make_smem_ptr(shared_tensors.smem_scale_A.data()), SmemLayoutScaleA{}); // BLK_M
      Tensor sScaleB = make_tensor(cute::make_smem_ptr(shared_tensors.smem_scale_B.data()), SmemLayoutScaleB{}); // BLK_N
      ...
      Tensor mScaleA_mkl = get<2>(load_inputs); // (BLK_M, m)
      Tensor mScaleB_nkl = get<3>(load_inputs); // (BLK_N, n)
    ...
    for (int k_tile_prologue = prologue_mma_count; k_tile_prologue > 0; --k_tile_prologue)
    {
      ...
      if (k_tile_count == 1) {
        ...
        Tensor sScaleAViewAsC = make_tensor(cute::make_smem_ptr(shared_tensors.smem_scale_A.data()),
            make_layout(make_shape(get<0>(sA.shape()), get<0>(sB.shape())), make_stride(1, 0)));
        Tensor tCsScaleAViewAsC = tiled_mma.get_slice(thread_idx).partition_C(sScaleAViewAsC);

        // (BLK_N) -> (BLK_M, BLK_N)
        Tensor sScaleBViewAsC = make_tensor(cute::make_smem_ptr(shared_tensors.smem_scale_B.data()),
            make_layout(make_shape(get<0>(sA.shape()), get<0>(sB.shape())), make_stride(0, 1)));
        Tensor tCsScaleBViewAsC = tiled_mma.get_slice(thread_idx).partition_C(sScaleBViewAsC);

        using RegLayoutScaleAViewAsC = decltype(make_layout_like(tCsScaleAViewAsC.layout())); // `make_layout_like` makes a compact layout.
        using RegLayoutScaleAEssential = decltype(filter_zeros(RegLayoutScaleAViewAsC{}.stride(), RegLayoutScaleAViewAsC{}.shape()));
        Tensor tCrScaleAViewAsC = make_tensor<ElementScale>(RegLayoutScaleAViewAsC{});
        
        // if (threadIdx.x == 128 && blockIdx.x == 0) {
        //     printf("tCsScaleAViewAsC \n");
        //     print(tCsScaleAViewAsC); // ((_2,_2,_16),_1,_1):((0,8,0),_0,_0)
        //     printf(" \n");
        // }
        CUTLASS_PRAGMA_UNROLL
        for (int i = 0; i < size(RegLayoutScaleAEssential{}); i++) {
            tCrScaleAViewAsC.data()[i] = tCsScaleAViewAsC(idx2crd(i, RegLayoutScaleAEssential{})) * tCsScaleBViewAsC(idx2crd(i, RegLayoutScaleAEssential{}));
        }
        accumulation.unsafe_scale(tCrScaleAViewAsC);
      } else {
